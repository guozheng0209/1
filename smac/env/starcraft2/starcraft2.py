...
class StarCraft2Env(MultiAgentEnv):
    ...
    def reward_battle(self):
        ...
        if self.reward_sparse:
            return 0
        new_reward = np.zeros(self.n_agents)
        reward = 0
        delta_deaths = 0
        delta_ally = 0
        delta_enemy = 0
        ...
        for al_id, al_unit in self.agents.items():
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    new_reward[al_id] += prev_health * neg_scale
                else:
                    # still alive
                    delta_ally += neg_scale * (
                            prev_health - al_unit.health - al_unit.shield
                    )
                    new_reward[al_id] += neg_scale * (
                            prev_health - al_unit.health - al_unit.shield)
        ...
        if self.reward_only_positive:
            ...
            for i in range(self.n_agents):
                new_reward[i] = delta_enemy + delta_deaths - new_reward[i]
        if self.reward_only_positive:
            return reward
        else:
            return new_reward
    ...
    def get_other_obs_agent(self, agent_id, other_id):
        unit = self.get_unit_by_id(agent_id)
        o_unit = self.get_unit_by_id(other_id)
        if o_unit.health > 0:  # otherwise dead, return all zeros
            o_x = unit.pos.x
            o_y = unit.pos.y
            o_sight_range = self.unit_sight_range(agent_id)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    enemy_feats[e_id, 1] = self.distance(o_x, o_y, e_x, e_y) / sight_range  # distance
                    enemy_feats[e_id, 2] = (
                                                   e_x - o_x
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if (
                    ...
                    ally_feats[i, 1] = self.distance(o_x, o_y, al_x, al_y) / sight_range  # distance
                    ally_feats[i, 2] = (al_x - o_x) / sight_range  # relative X
                    ally_feats[i, 3] = (al_y - o_y) / sight_range  # relative Y
    ...
    def get_obs_size(self):
        ...
        print(f"move_feats size: {move_feats}")
        print(f"enemy_feats size: {n_enemies * n_enemy_feats}")
        print(f"ally_feats size: {n_allies * n_ally_feats}")
        print(f"own_feats size: {own_feats}")
        print(
            f"Total calculated size: {move_feats + (n_enemies * n_enemy_feats) + (n_allies * n_ally_feats) + own_feats}")
