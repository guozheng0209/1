...
class ReplayBuffer:
    ...
    def reset_buffer(self):
        self.buffer = {'obs_n': np.zeros([self.batch_size, self.episode_limit, self.N, self.obs_dim]),
                       ...
                       # graph
                       's': np.zeros([self.batch_size, self.episode_limit, self.N,self.state_dim]),
                       ...
                       'share_obs': np.zeros([self.batch_size, self.episode_limit, self.N, self.obs_dim]),

                       }
        self.episode_num = 0
        self.max_episode_len = 0
    #graph
    def store_transition(self, episode_step, obs_n, s, v_n, avail_a_n, a_n, a_logprob_n, r, dw,share_obs):
    # def store_transition(self, episode_step, obs_n, s, v_n, avail_a_n, a_n, a_logprob_n, r, dw):
        ...
        #graph
        self.buffer['share_obs'][self.episode_num][episode_step] = share_obs
