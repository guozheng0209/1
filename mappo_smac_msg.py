...
class Actor_RNN(nn.Module):
    def __init__(self, args, actor_input_dim):
        ...
        self.msg_fc1 = nn.Linear(actor_input_dim, args.rnn_hidden_dim)
        self.msg_rnn = nn.GRUCell(args.rnn_hidden_dim, args.rnn_hidden_dim)
        self.msg_fc2 = nn.Linear(args.rnn_hidden_dim, args.action_dim)
        self.msg_activate_func = [nn.Tanh(), nn.ReLU()][args.use_relu]
        ...
        if args.use_orthogonal_init:
            ...
            print("------use_orthogonal_init------")
            orthogonal_init(self.msg_fc1)
            orthogonal_init(self.msg_rnn)
            orthogonal_init(self.msg_fc2, gain=0.01)

    def forward(self, actor_input, msg, avail_a_n):
        ...
        msg_x = self.msg_activate_func(self.msg_fc1(msg))


        self.msg_rnn_hidden = self.msg_rnn(msg_x, self.msg_rnn_hidden)

        msg_x = self.msg_fc2(self.msg_rnn_hidden)


        # x = self.fc2(torch.cat([self.rnn_hidden, self.msg_rnn_hidden], dim=-1))
        # msg_x = self.fc2(self.msg_rnn_hidden)
        # print(x.shape)
        # print(msg_x.shape)
        # print(1)
        x = x + 0.3*msg_x
        # x = self.fc3(torch.cat([x, msg_x], dim=-1))
...
class Critic_RNN(nn.Module):
    def __init__(self, args, critic_input_dim,actor_input_dim):
        ...
        self.msg_rnn = nn.GRUCell(args.rnn_hidden_dim, args.rnn_hidden_dim)
        self.msg_activate_func = [nn.Tanh(), nn.ReLU()][args.use_relu]
        self.msg_fc2 = nn.Linear(args.rnn_hidden_dim, 1)
        ...
        if args.use_orthogonal_init:
            ...
            orthogonal_init(self.msg_fc1)
            orthogonal_init(self.msg_rnn)
            orthogonal_init(self.msg_fc2)

    def forward(self, critic_input, msg):
...
class MAPPO_SMAC:
    def __init__(self, args):
        ...
        if self.use_rnn:
            ...
            self.critic = Critic_RNN(args, self.critic_input_dim, self.actor_input_dim)
            if args.cuda:
    ...
    def choose_action(self, obs_n, msg, avail_a_n, evaluate):
        with torch.no_grad():
            actor_inputs = []
            ...
            if self.args.cuda:
                ...
                msg = msg.to(self.args.device)
            prob = self.actor(actor_inputs, msg, avail_a_n)  # prob.shape=(N, action_dim)
    ...
    def get_value(self, s, obs_n, msg):
        with torch.no_grad():
            critic_inputs = []
            ...
            msg = torch.tensor(msg, dtype=torch.float32) # (state_dim,)-->(N,state_dim)
            #s = [s]*self.N
            ...
            v_n = self.critic(critic_inputs, msg)  # v_n.shape(N,1)
    ...
    def train(self, replay_buffer, total_steps):
        ...
        actor_inputs, critic_inputs, msg = self.get_inputs(batch, max_episode_len)
        if self.args.cuda:
        ...
        for _ in range(self.K_epochs):
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                if self.use_rnn:
                    ...
                    self.actor.msg_rnn_hidden = None
                    self.critic.rnn_hidden = None
                    self.critic.msg_rnn_hidden = None
                    probs_now, values_now = [], []

                    for t in range(max_episode_len):
                        # prob.shape=(mini_batch_size*N, action_dim)

                        prob = self.actor(actor_inputs[index, t].reshape(self.mini_batch_size * self.N, -1), msg[index, t].reshape(self.mini_batch_size * self.N,-1),
                        ...
                        v = self.critic(critic_inputs[index, t].reshape(self.mini_batch_size * self.N, -1), msg[index, t].reshape(self.mini_batch_size * self.N,-1))  # v.shape=(mini_batch_size*N,1)
                    ...
                    probs_now = self.actor(actor_inputs[index], msg[index], avail_a_n[index])
                    values_now = self.critic(critic_inputs[index], msg[index]).squeeze(-1)
    ...
    def get_inputs(self, batch, max_episode_len):
        actor_inputs, critic_inputs, msg = [], [], []




        actor_inputs.append(batch['obs_n'])
        msg.append(batch['msg'])
       ...
       # critic_inputs.append(batch['s'].unsqueeze(2).repeat(1, 1, self.N, 1))
        ...
        msg = torch.cat([x for x in msg], dim=-1)


        return actor_inputs, critic_inputs, msg
    ...
    def load_model(self, env_name, number, seed, step):
        self.actor.load_state_dict(torch.load("./model/MAPPO_env_{}_actor_number_{}_seed_{}_step_{}k.pth".format(env_name, number, seed, step)), False)
